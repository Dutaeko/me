...
public class IsekaiKomik extends KomikcastClient {
    public static final String SOURCE_ID = MangaSettingsManager.MANGA_SOURCE_ISEKAIKOMIK;
    protected static String base() { return MangaSettingsManager.getSourceDomain(SOURCE_ID); }
    private static final String LABEL = "Isekai Komik";
            ...
            getDocument(url, new Result<Document>() {
                @Override public void onSuccess(Document document, boolean ignored) {
                    MangaCoroutines.io(() -> {
                        ...
                        } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Daftar Isekai Komik gagal dibaca")); }
                    });
                }
    ...
    @Override public void detail(String slug, Result<MangaPost> cb) {
        ...
        if (clean.isEmpty()) { cb.onError("Slug Isekai Komik kosong"); return; }
            ...
            @Override public void onSuccess(Document document, boolean ignored) {
                MangaCoroutines.io(() -> {
                    try {
                        ...
                        if (post.title.trim().isEmpty()) { MangaCoroutines.main(() -> cb.onError("Detail Isekai Komik kosong")); return; }
                    ...
                    } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Detail Isekai Komik gagal dibaca")); }
                });
            }
            @Override public void onError(String message) { cb.onError(message); }
        });
            ...
            @Override public void onSuccess(Document document, boolean ignored) {
                MangaCoroutines.io(() -> {
                    try {
                        ...
                        if (pages.isEmpty()) { MangaCoroutines.main(() -> cb.onError("Halaman Isekai Komik kosong")); return; }
                        PAGE_CACHE.put(key, new ArrayList<>(pages));
                        MangaCoroutines.main(() -> cb.onSuccess(pages, false));
                    } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Halaman Isekai Komik gagal dibaca")); }
                });
            }
            @Override public void onError(String message) { cb.onError(message); }
        });
    ...
    private void loadPagesFromCandidates(ArrayList<String> urls, int pos, String key, Result<ArrayList<String>> cb) {
        if (urls == null || pos >= urls.size()) { cb.onError("Chapter Isekai Komik tidak ditemukan"); return; }
    ...
    private String buildListUrl(int page, String sort, String query, String genre) throws Exception {
        ...
        boolean completedSort = "completed".equals(s) || "complete".equals(s);
        if (filter.status.isEmpty() && completedSort) filter.status = "completed";
        if (completedSort && "completed".equals(filter.status) && filter.genres.isEmpty() && filter.type.isEmpty()) {
            HttpUrl completedUrl = HttpUrl.parse(base() + "/manga/");
            if (completedUrl != null) {
                HttpUrl.Builder builder = completedUrl.newBuilder();
                if (page > 1) builder.addQueryParameter("page", String.valueOf(page));
                builder.addQueryParameter("status", "completed");
    ...
    private ArrayList<MangaPost> parseList(Document document) {
        ...
        Elements items = document.select(".seriesearch .listupd .bsx, .postbody .listupd > .bs > .bsx, main .listupd > .bs > .bsx, article .listupd > .bs > .bsx, .manga-list .manga-item, a.manga-item, .manga-item");
        if (items.isEmpty()) items = document.select(".listupd > .bs > .bsx, .bsx");
    ...
    private MangaPost parseDetail(String clean, Document document) {
        String title = firstNonEmpty(text(document, ".seriestuheader h1.entry-title"), text(document, "h1.entry-title"), text(document, ".entry-title"), document.title() == null ? "" : document.title().replace("- IsekaiKomik", "").replace("| IsekaiKomik", "").replace("Baca Manga", "").trim());
        String cover = firstNonEmpty(imageUrl(document.selectFirst(".thumb img, .info-left .thumb img, .main-info .thumb img, article img[itemprop=image], img.wp-post-image")), attr(document.selectFirst("meta[property=og:image], meta[name=twitter:image]"), "content"));
        String synopsis = cleanSynopsis(firstNonEmpty(text(document, ".seriestuhead .entry-content.entry-content-single"), text(document, ".info-desc .entry-content.entry-content-single"), text(document, ".entry-content.entry-content-single[itemprop=description]"), text(document, ".entry-content.entry-content-single p"), text(document, ".entry-content-single")));
        String genre = joinTexts(document.select(".seriestugenre a[href*='/genres/'], .seriestugenre a[rel=tag], .mgen a, span.mgen a"));
        ...
        String author = cleanInfoValue(infoValue(document, "Author"));
        MangaPost post = new MangaPost(clean, title, cover, author, cleanInfoValue(status), synopsis, genre, cleanInfoValue(type)).withSource(SOURCE_ID, LABEL);
    ...
    private void getDocument(Request req, Result<Document> cb) {
        CloudflareHelper.enqueue(client, req, sourceLabel(), new Callback() {
            ...
            @Override public void onResponse(Call call, Response response) throws IOException {
                ...
                } catch(Exception e) { main.post(() -> cb.onError("Data Isekai Komik gagal dibaca")); }
            }
        });
    }
    ...
    private String detailInfo(Document document, String genre) {
        ...
        addInfo(values, "Alternative", firstNonEmpty(text(document, ".seriestualt"), text(document, ".alternative"), infoValue(document, "Alternative"), infoValue(document, "Alternative Name")));
    ...
    private String cleanInfoValue(String raw) {
        ...
        String lower = value.toLowerCase(Locale.ROOT);
        if ("?".equals(value) || "-".equals(value) || "--".equals(value) || "n/a".equals(lower) || "na".equals(lower) || "null".equals(lower) || "unknown".equals(lower) || "updating".equals(lower)) return "";
        return value;
    }
    ...
    private boolean isNoiseGenre(String title) {
        ...
        return value.isEmpty() || value.equals("genres") || value.equals("genre") || value.equals("manga") || value.equals("isekaikomik");
    }
    ...
    private String[][] fallbackGenrePairs() {
        return new String[][]{{"Action", "action"}, {"Adaptation", "adaptation"}, {"Adult", "adult"}, {"Adult Cast", "adult-cast"}, {"Adventure", "adventure"}, {"Aliens", "aliens"}, {"Animals", "animals"}, {"Blackmail", "blackmail"}, {"Bloody", "bloody"}, {"Comedy", "comedy"}, {"Cooking", "cooking"}, {"Crime", "crime"}, {"Cultivation", "cultivation"}, {"Death Game", "death-game"}, {"Delinquents", "delinquents"}, {"Demons", "demons"}, {"Drama", "drama"}, {"Ecchi", "ecchi"}, {"Fantasy", "fantasy"}, {"Full Color", "full-color"}, {"Game", "game"}, {"Games", "games"}, {"Gender Bender", "gender-bender"}, {"Gore", "gore"}, {"Harem", "harem"}, {"Historical", "historical"}, {"Horror", "horror"}, {"Isekai", "isekai"}, {"Josei", "josei"}, {"Magic", "magic"}, {"Manhua", "manhua"}, {"Manhwa", "manhwa"}, {"Martial Arts", "martial-arts"}, {"Mature", "mature"}, {"Medical", "medical"}, {"Military", "military"}, {"Monsters", "monsters"}, {"Murim", "murim"}, {"Mystery", "mystery"}, {"Psychological", "psychological"}, {"Regression", "regression"}, {"Reincarnation", "reincarnation"}, {"Romance", "romance"}, {"School", "school"}, {"School Life", "school-life"}, {"Sci-Fi", "sci-fi"}, {"Seinen", "seinen"}, {"Shoujo", "shoujo"}, {"Shounen", "shounen"}, {"Slice of Life", "slice-of-life"}, {"Sports", "sports"}, {"Supernatural", "supernatural"}, {"Survival", "survival"}, {"Thriller", "thriller"}, {"Time Travel", "time-travel"}, {"Tragedy", "tragedy"}, {"Villainess", "villainess"}, {"Wuxia", "wuxia"}};
    }

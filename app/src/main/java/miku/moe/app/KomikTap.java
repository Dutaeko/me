...
public class KomikTap extends KomikcastClient {
    public static final String SOURCE_ID = MangaSettingsManager.MANGA_SOURCE_KOMIKTAP;
    protected static String base() { return MangaSettingsManager.getSourceDomain(SOURCE_ID); }
    private static final String LABEL = "KomikTap";
            ...
            getDocument(url, new Result<Document>() {
                @Override public void onSuccess(Document document, boolean ignored) {
                    MangaCoroutines.io(() -> {
                        ...
                        } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Daftar KomikTap gagal dibaca")); }
                    });
                }
    ...
    @Override public void detail(String slug, Result<MangaPost> cb) {
        ...
        if (clean.isEmpty()) { cb.onError("Slug KomikTap kosong"); return; }
            ...
            @Override public void onSuccess(Document document, boolean ignored) {
                MangaCoroutines.io(() -> {
                    try {
                        ...
                        if (post.title.trim().isEmpty()) { MangaCoroutines.main(() -> cb.onError("Detail KomikTap kosong")); return; }
                    ...
                    } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Detail KomikTap gagal dibaca")); }
                });
            }
            @Override public void onError(String message) { cb.onError(message); }
        });
            ...
            @Override public void onSuccess(Document document, boolean ignored) {
                MangaCoroutines.io(() -> {
                    try {
                        ...
                        if (pages.isEmpty()) { MangaCoroutines.main(() -> cb.onError("Halaman KomikTap kosong")); return; }
                        PAGE_CACHE.put(key, new ArrayList<>(pages));
                        MangaCoroutines.main(() -> cb.onSuccess(pages, false));
                    } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Halaman KomikTap gagal dibaca")); }
                });
            }
            @Override public void onError(String message) { cb.onError(message); }
        });
    ...
    private void loadPagesFromCandidates(ArrayList<String> urls, int pos, String key, Result<ArrayList<String>> cb) {
        if (urls == null || pos >= urls.size()) { cb.onError("Chapter KomikTap tidak ditemukan"); return; }
    ...
    private ArrayList<MangaPost> parseList(Document document) {
        ...
        for (Element item : items) {
            ...
            if (link == null || link.absUrl("href").contains("/manga/list-mode")) continue;
            ...
            String type = parseType(item.selectFirst(".limit .type, .type"));
    ...
    private MangaPost parseDetail(String clean, Document document) {
        String title = firstNonEmpty(text(document, ".postbody.seriestu h1.entry-title"), text(document, "h1.entry-title"), document.title() == null ? "" : document.title().replace("| Komiktap", "").replace("| KomikTap", "").trim());
        String cover = imageUrl(document.selectFirst(".postbody.seriestu .thumb img, .thumb img, article img[itemprop=image], img.wp-post-image"));
        String synopsis = firstNonEmpty(text(document, ".entry-content.entry-content-single[itemprop=description]"), text(document, ".entry-content.entry-content-single"), text(document, ".entry-content"));
        String genre = joinTexts(document.select(".seriestugenre a[rel=tag], .seriestugenre a[href*='/genres/'], .mgen a[href*='/genres/']"));
    ...
    private ArrayList<String> parsePages(Document document, String chapterUrl) {
        ...
        Matcher direct = Pattern.compile("https?:\\/\\/wibulep\\.xyz\\/[^\\\"\'<>\\s)]+", Pattern.CASE_INSENSITIVE).matcher(html);
        while (direct.find()) addPage(out, seen, direct.group().replace("\\/", "/"), chapterUrl);

        if (out.isEmpty()) {
            Elements images = document.select("#readerarea img, .readerarea img, .chapterbody img, .entry-content-single img, .entry-content img, .maincontent img");
    ...
    private void addPage(ArrayList<String> out, LinkedHashSet<String> seen, String raw, String chapterUrl) {
        ...
        if (!lower.matches(".*\\.(jpg|jpeg|png|webp)(?:\\?.*)?$") && !lower.contains("/manga-images/")) return;
        if (seen.add(url)) {
    ...
    private void getDocument(Request req, Result<Document> cb) {
        CloudflareHelper.enqueue(client, req, sourceLabel(), new Callback() {
            ...
            @Override public void onResponse(Call call, Response response) throws IOException {
                ...
                } catch(Exception e) { main.post(() -> cb.onError("Data KomikTap gagal dibaca")); }
            }
        });
    }
    ...
    private String knownGenreId(String raw) {
        ...
        if ("action".equals(value)) return "107";
        if ("adult".equals(value)) return "2";
        if ("adventure".equals(value)) return "108";
        if ("comedy".equals(value)) return "3";
        if ("drama".equals(value)) return "16";
        if ("ecchi".equals(value)) return "4";
        if ("fantasy".equals(value)) return "109";
        if ("harem".equals(value)) return "20";
        if ("isekai".equals(value)) return "985";
        if ("romance".equals(value)) return "6";
        if ("school".equals(value)) return "2487";
        if ("school-life".equals(value)) return "7";
        if ("seinen".equals(value)) return "8";
        if ("shounen".equals(value)) return "13";
        if ("slice-of-life".equals(value)) return "14";
        if ("smut".equals(value)) return "285";
        if ("supernatural".equals(value)) return "9";
        if ("vanilla".equals(value)) return "589";
        return "";
    }
    ...
    private boolean isNoiseGenre(String title) {
        ...
        return value.isEmpty() || value.equals("genres") || value.equals("genre") || value.equals("manga") || value.equals("komiktap");
    }

    private ArrayList<GenreItem> fallbackGenres() {
        ArrayList<GenreItem> out = new ArrayList<>();
        String[][] values = { {"Action", "107"}, {"Adult", "2"}, {"Adventure", "108"}, {"Comedy", "3"}, {"Drama", "16"}, {"Ecchi", "4"}, {"Fantasy", "109"}, {"Harem", "20"}, {"Isekai", "985"}, {"Romance", "6"}, {"School", "2487"}, {"School Life", "7"}, {"Seinen", "8"}, {"Shounen", "13"}, {"Slice of Life", "14"}, {"Smut", "285"}, {"Supernatural", "9"}, {"Vanilla", "589"}, {"Manga", "type:manga"}, {"Manhwa", "type:manhwa"}, {"Manhua", "type:manhua"} };

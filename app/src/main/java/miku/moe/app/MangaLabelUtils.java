...
public final class MangaLabelUtils {
    ...
    public static CharSequence favoriteChapterIncreaseLabel(int base, int added) {
        return favoriteChapterIncreaseLabel(base, added, null);
    }

    public static CharSequence favoriteChapterIncreaseLabel(int base, int added, Context context) {
        if (base <= 0 || added <= 0) return "";
        String text = base + "+" + added;
        SpannableString spannable = new SpannableString(text);
        int start = String.valueOf(base).length();
        int increaseColor = context == null
                ? 0xFFFF2D2D
                : ThemeManager.color(context, com.google.android.material.R.attr.colorError, 0xFFFF2D2D);
        spannable.setSpan(new ForegroundColorSpan(increaseColor), start, text.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        return spannable;
    }

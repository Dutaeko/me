...
public final class ThemeManager {
    ...
    /**
     * Dark/light mode follows the device. It used to be pinned to MODE_NIGHT_YES, which
     * meant a phone in light mode still got the hard-coded night palette.
     */
    public static void applyDeviceNightMode(Context context) {
        AppCompatDelegate.setDefaultNightMode(AppCompatDelegate.MODE_NIGHT_FOLLOW_SYSTEM);
    }

    /** @deprecated use {@link #applyDeviceNightMode(Context)} */
    @Deprecated
    public static void applyNightMode(Context context) {
        applyDeviceNightMode(context);
    }

    /** True when the user picked "Default" and the device can provide a dynamic palette. */
    public static boolean isDeviceThemeEnabled(Context context) {
        return THEME_DEFAULT.equals(getTheme(context)) && DynamicColors.isDynamicColorAvailable();
    }

    /** Convenience accessor so call sites never hard-code a colour. */
    ...
    public static void applyTheme(Activity activity) {
        applyDeviceNightMode(activity);
        activity.setTheme(getThemeStyle(activity));
        // MikuMoeApplication installs DynamicColors through a lifecycle callback, but the
        // setTheme() above re-asserts the static palette and silently wipes it. Re-apply the
        // device palette *after* it so Material You actually reaches the UI for the "Default"
        // theme. A manually picked AppTheme.* keeps its own colours on purpose.
        if (isDeviceThemeEnabled(activity)) {
            DynamicColors.applyToActivityIfAvailable(activity);
        }
        int surfaceColor = color(activity, com.google.android.material.R.attr.colorSurface, 0);
    ...
    public static void applySystemBars(Activity activity) {
        ...
        // Bar icon appearance follows the *actual* surface luminance instead of the uiMode
        // flag, so a light dynamic palette never ends up with light (unreadable) icons.
        applyBarAppearance(window, MaterialColors.isColorLight(surfaceColor));
    }

    private static void applyBarAppearance(Window window, boolean lightBars) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R && window.getInsetsController() != null) {
            int appearance = WindowInsetsController.APPEARANCE_LIGHT_STATUS_BARS
                    | WindowInsetsController.APPEARANCE_LIGHT_NAVIGATION_BARS;
            window.getInsetsController().setSystemBarsAppearance(lightBars ? appearance : 0, appearance);
            return;
        }
    ...
    /**
     * A context that carries the currently active theme. Activities already own the dynamic
     * (Material You) overlay, so they are returned untouched; anything else falls back to the
     * application context with the selected app theme applied.
     */
    public static Context createThemedContext(Context context) {
        if (context instanceof Activity) return context;
        Context themed = context.getApplicationContext();
        themed.setTheme(getThemeStyle(themed));
        return themed;
    }

    public static int getThemedColor(Context context, int attr, int fallback) {
        return MaterialColors.getColor(createThemedContext(context), attr, fallback);
    }

    /** @deprecated use {@link #createThemedContext(Context)} - night mode is no longer forced. */
    @Deprecated
    public static Context createForcedNightContext(Context context) {
        return createThemedContext(context);
    }

    /** @deprecated use {@link #getThemedColor(Context, int, int)}. */
    @Deprecated
    public static int getForcedNightColor(Context context, int attr, int fallback) {
        return getThemedColor(context, attr, fallback);
    }
    ...
    public static void applyWebViewSystemBars(Activity activity) {
        ...
        int surfaceColor = getThemedColor(activity, com.google.android.material.R.attr.colorSurface, Color.BLACK);
        window.setBackgroundDrawable(new ColorDrawable(surfaceColor));
        window.setStatusBarColor(surfaceColor);
